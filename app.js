...
(function () {
  ...
  (function () {
    ...
    var STEPS = [
      [28, 'Готуємо арену сезону…'],
      [54, 'Збираємо команди майстринь…'],
      [78, 'Кличемо журі та експертів…'],
      [96, 'Вмикаємо камери й ефіри…'],
      [100, 'Готово. Ваш хід — крок 1 нижче.']
    ];
  ...
  if (mods) {
    ...
    var paintMods = function () {
      ...
      if (allBtn) {
        allBtn.textContent = unlocked < items.length
          ? 'Відкрити модуль ' + num(unlocked - (items[unlocked - 1].classList.contains('open') ? 0 : 1))
          : (open === items.length ? 'Згорнути всі' : 'Відкрити всі');
      }
      if (hintEl) {
        hintEl.textContent = unlocked < items.length
          ? 'Модулі відкриваються по черзі — так само, як на сезоні. Далі модуль ' + num(unlocked - 1 + (items[unlocked - 1].classList.contains('open') ? 1 : 0)) + '.'
    ...
    mods.addEventListener('click', function (e) {
      ...
      if (hintEl) hintEl.textContent = 'Спочатку відкрийте модуль ' + num(unlocked - 1) + ' — далі відкриється наступний.';
    }, true);
