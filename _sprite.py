...
def build_sprite():
    ...
    for name in sorted(PX):
        parts.append('  <symbol id="i-px-%s" viewBox="0 0 16 16" shape-rendering="crispEdges">'
                     '<g fill="currentColor">%s</g></symbol>' % (name, pattern_to_svg(PX[name])))
    parts.append('</svg>')
    return "\n".join(parts)
  ...
  <div class="px-win pl-win">
    ...
    <div class="px-win-body">
      ...
      <div class="pl-start" id="plStart" hidden>
        <div class="pl-mark" aria-hidden="true"><svg viewBox="0 0 16 16"><use href="#i-px-star"/></svg></div>
